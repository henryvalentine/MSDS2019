...
processNodes <- function(rawNode, category)
{
    imgs <- vector()
    categories <- vector()
    titles <- vector()
    ...
    for(i in 1: length(itemsRaw))
    {
        ...
        {
          xTitle <- str_split(xTitle, ':')[[1]][1] # Shorten the title
        }
    ...
    return(data.frame(title = titles, author = authors, category = categories, rating = ratings, img = imgs,
...
```{r}
books <- rbind(religion, science, fiction, history, law)
dim(books)
```
