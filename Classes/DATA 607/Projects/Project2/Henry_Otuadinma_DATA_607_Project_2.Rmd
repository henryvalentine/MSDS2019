...
The top 5 best seller and highly rated books across 5 different genres (History, Law, Literature & Fiction, Science and Math, and Religion) scrapped from Amazon. I want to purchase the cheapest 5 (one from each genre) among them.
...
The data will contain descriptive observations in a fairly clean form because specific information were look for on the web pages. Each book will have eight variables depicting its title, author, genre, rating, price, type (kindle edition, paperback, etc), etc. 

The data will then be worked on using appropriate functions like `group_by`, `filter`, `arrange`, `select`, etc from the `dplyr` package to see how the books compare with each others in terms of their genres, types, prices, and the most rated such that at the end, I will be able to to know the cheapest 5 top rated and best selling books (one each from the 5 genres) that I can purchase on Amazon.
...
ggplot(books2, aes(x=genre, y=reviewers, fill=genre))+
  scale_fill_manual(values = c("steelblue", "#26868d", '#D32F2F', '#7B1FA2', '#3F51B5') ) + 
...
ggplot(s4, aes(x=Fiscal_Year, y=Total, fill=Fiscal_Year))+
  scale_fill_manual(values = c("steelblue", "#26868d", '#440154', '#7B1FA2', '#3F51B5', '#D32F2F', '#00695c', '#303f9f', '#6acb5c', '#2e748e', '#6acb5c','#45bc72' ) ) + 
...
ggplot(s3, aes(x = Fiscal_Year, y = TotalValue, fill = Month)) + 
  ...
  scale_fill_manual(values = c("steelblue", "purple", "#440154", "#45367f", "#26868d", '#440154', '#35618d', '#2e748e', '#2dab81', '#2e748e', '#45367f', '#3d4c89', '#6acb5c', '#2e748e', '#6acb5c','#45bc72') )
...
ggplot(s5, aes(x = Fiscal_Year, y = Total, fill = Month)) + 
  ...
  scale_fill_manual(values = c("steelblue", "#26868d", "#440154", "#45367f", "#45bc72", '#6acb5c', '#35618d', '#6acb5c', '#2dab81', '#2e748e', '#45367f', '#3d4c89') ) # + coord_flip()
```
...
#ggplot(temp, aes(x = minute, y = temperature)) + geom_line()
 ...
 ggplot(heating, aes(x = minute, y = temperature, fill = color)) + 
  ...
  scale_fill_manual(values = c("black", "#6acb5c", "#440154", "#45367f", "#D32F2F") )
...
##### From all indications, Black colors tend tend to absorb heat the most (very fast) and slowest in releasing (emitting) heat. White colors on the other hand tend to be slowest in absorbing heat and very fast in cooling down (releasing/emiting heat). Other colors fall in-between these two.