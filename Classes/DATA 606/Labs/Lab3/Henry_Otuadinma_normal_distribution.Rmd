...
```{r}
hist(mdims$hgt, xlab = "Men's height", main = "Plot of Men's height agains frequency")
...
```{r}
hist(fdims$hgt, xlab = "Women's height", main = "Plot of Women's height agains frequency")
```
...
Answer: The mean heights for men and women are 177.7cm and 164.9cm respectively
Their Interquartile ranges are 9.8 (182.7 - 172.9) and 9.5 (169.5  - 160.0) respectively
Both are unimodal, normally distributed, and don't have a particular skew
...
Answer: Yes the data follows a nearly normal distribution
...
Majority of the points fall on the plot line but few outliers lie at both tails of the line.
In comparison with the probability of the real data, more points fall on the plot line but bot plots show similar characteristics
...
Answer:
For each plots, the points fall along the lines but they follow the line more closely in the plot for the simulated data
...
```{r wsim-norm, eval=TRUE}
wsim_norm <- rnorm(n = length(fdims$wgt), mean = mean(fdims$wgt), sd = sd(fdims$wgt))
```
...
```{r qqnormsimw, eval=TRUE}
qqnormsim(fdims$wgt)
```

The histogram plot for the females weigth is right skewed and unimodal.
The probability plot shows many of the points falling away from the plot line which depicts the presence of many outliers.
The plot of the simulated values for the females' weight shows a more normal distribution as most of the points fall on the plot line with few outliers at both tails of the line.
These depict that the females' weight is not actually a normal distribution.
...
What is the probability that a randomly selected female is above 152.4 cm(5 feet)?
...
What is the probability that the weight of a randomly selected female is between 60kg and 70kg?
 
```{r}
fMeanWgt <- mean(fdims$wgt)
fWgtSd   <- sd(fdims$wgt)

 pnorm(q=70,mean=fMeanWgt,sd=fWgtSd) - pnorm(q=60,mean=fMeanWgt,sd=fWgtSd)
```

```{r}
sum(fdims$wgt > 60 & fdims$wgt < 70)/ length(fdims$wgt)

```
    
From these computations, it can be observed that the females' emperical height distribution closely agrees with the real height distribution Therefore, it can be said that the females' height follows a normal distribution.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    Answer:
    The stepwise patterns are due to the fact that both variables are most discrete in nature. 
...
From the normal probability plot above, the variable `(fdims$kne.di)` for females is right skewed and can be 
verified from the histogram shown below
