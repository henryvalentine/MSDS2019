---
title: "DATA606 Lab7: Introduction to linear regression"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
####  A scatterplot using `runs` and `at_bats` as the explanatory variable would be more suitable for this.
...
Answer:
The relationship between the variables appear to have deviations and the plot points are not centred around any particular value, There are also few outliers observed at the upper part. It appears that most runs occur between 0 to 5600 `at_bats` and $(0.610627)^2$ shows a positive but not very strong corelation between them.
...
#### The smallest sum of squares I got is: Sum of Squares:  123721.9
...
#### The slope tells that `homeruns` have a greater impacts on `runs` than `at_bats`
...
(estimated = -2789.2429+(0.6305*5578))

```

```{r}

(residual = mlb11$runs[mlb11$at_bats==5579] - estimated)

```

##### She over-estimated the runs by a value of 14.6861, approximately 15
...
#### It appears most of the residuals are centered around 0, which shows a linear relationship.
...
##### Yes, the residuals appear to be normal.
...
##### Let's see what will be the relationship of `bat_avg` with `runs`
...
```{r}
plot_ss(y = mlb11$runs, x = mlb11$bat_avg)
...
##### A positive linear relationship seems to exist between `bat_avg` and `runs`
...
```{r}
(R <- cor(mlb11$runs, mlb11$bat_avg))
```
...
`bat_avg` appears to predict `runs` better than `at_bats` because the $R^2$ is siginificantly higher than that of `at_bats`
...
rSquares <- variables %>%
  map(.f = function(x) {
    ...
    df <- tibble(variable = c(x), r2 = c(summary(model)$r.squared))
    return(df)
  }) %>%
...
```{r}
variables2 <- mlb11 %>% select(new_onbase:new_obs) %>% names()
...
newObsModel <- lm(runs ~ new_obs, data = mlb11)

# representing the linear relationship
ggplot(mlb11, aes(x = new_obs, y = newObsModel$residuals)) +
  geom_point() +
  geom_hline(yintercept = 0, linetype = 3)

```

#### The relationship looks linear as I cannot see any curvatures of the line or abnormalities


```{r}
hist(newObsModel$residuals)

```

####  From the histogram, we can say that the residuals are approximately normal

```{r}
qqnorm(newObsModel$residuals)
qqline(newObsModel$residuals)

```

#### The normality of the residual data appears to be confirmed by the fact that the distribution very closely looks like a straight line, while the scatter plot itself appears to show that constant variability is met
