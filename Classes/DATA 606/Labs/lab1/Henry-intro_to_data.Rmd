---
title: "DATA 606 Lab1: Introduction to data"
output:
  html_document:
    theme: cerulean
    highlight: pygments
...
```{r}
paste0("There are ", nrow(cdc), " cases in the data set")
```

### How many variables? 
```{r}

paste0("There are ", ncol(cdc), " variables in the data set")
...
The numerical summary of height:
```{r eval=TRUE}
summary(cdc$height)
```

The numerical summary of gae:
...
```{r}
paste0("Interquartile Range for height: ", 70-64)
```

```{r}
paste0("Interquartile Range for age: ", 57-31)
```
...
cat("Relative frequency distribution for gender: ")
table(cdc$gender)/20000

```

```{r}
cat("Relative frequency distribution for exerany: ")
...
```{r}
paste0("the number of males in the sample is: ", count(subset(cdc, gender == 'm')))
```

### What proportion of the sample reports being in excellent health?

```{r}
paste0("the proportion of the sample that reports being in excellent health is: ", count(subset(cdc, genhlth == 'excellent')))
```
...
Answer:
Males tend to indulge in smoking more than females. They can indulge very early in their lives or 
they can smoke a lot within a short time
...
```{r}
head((cdc$weight / cdc$height^2) * 703, 30)
```
...
Answer:
The plot indicates that BMI is related to health condition such that it tends to be lowest
when the individual is in excellent health, but increases if the health status deteroriates.
This might mean that those in excellent health tend to have normal weight while obesity can be 
the case for those whose health status are either fair or poor.


### Pick another categorical variable from the data set and see how it relates to BMI. List the 
### variable you chose, why you might think it would have a relationship to BMI,  and indicate what the 
### figure seems to suggest. 

Answer:

Ploting BMI against exerany
I think exerany will have a relationship with BMI because exercises helps to keep the body in shape
The figure suggests that those that have exercised within the past one month are more likely to be 
in better shape as the median and Interquertile range of their plot are lower than those of the opposite
...
Answer:
The first histogram merged a lot of information/representation in one bin for the 9 bins presented, but the second one
with the 50 argument presented the information clearer such that more data points are visible on the plot
...
```{r}
plot(x=cdc$wtdesire, y=cdc$weight, type = 'p',xlab='Desired Weigth', ylab='Weight', main='Weight versus Desired Weight')    
    
```

This plot shows that there are not much differences between the weight and desired weight of a lot of the cases.
Most of them are either comfortable with their present weight or working on 
attaining it.
...
###     positive or negative?
    ...
    wdiff is a Numerical (continuous)
    
    if wdiff is 0, It means the person is already at the desired weight or is comfortable with the current 
    weight
    
    if wdiff is positive, it means the person wants to add weight and hasn't attained the 
    desired weight (the desired weight is higher than the current weight)
    
    if wdiff is negative, it means the person wants to lose weight. Their current weight is higher than the desired weight 
    and has not been attained
...
boxplot(wdiff, main='Box plot of wdiff')

```

```{r}

hist(wdiff, breaks = 50, xlab = 'wdiff', ylab='Frequency')

```

```{r}
plot(wdiff, type='p',  xlab = '', ylab='wdiff', main='Scatter plot of wdiff')

```

From the shape of the histgram (unimodal), many people are comfortable with their weight while few want to lose weight.
From the slight left skew, it appears more people want to lose weight than gain weight.
From the summary, the most peolple want to loose approximately 10 - 15 pounds of weight, while the IQR is lies between 0.00 and -21.00
...
cat('Weight summary for males', sep = '\n')
summary(cdc[cdc$gender == 'm',]['weight'])

```

```{r}
cat('weight summar for females: ', '\n')
summary(cdc[cdc$gender == 'f',]['weight'])

```

```{r}
boxplot(cdc$weight ~ cdc$gender, ylab='Weight', xlab='Gender', main='Weight vs. Gender')

```

These summaries and box plots indicated that males tend to be weightier than the females. 
This can be due to muscular weight. The IQR for males is larger than that for females and 
so is the mean, median, maximum and minimum weights. It is apparent that males want to gain 
...
weightStd <- sd(cdc$weight)
paste0('The mean and Standard deviation of weight are: ', meanWeight, ' and ', weightStd, ' respectively')

```

```{r}
weightsInStd <- cdc[cdc$weight < (meanWeight + weightStd) & cdc$weight > (meanWeight - weightStd),]
head(weightsInStd)

```

```{r}
paste0('Proportion of weight in standard deviation is:  ', dim(weightsInStd)[1]/dim(cdc)[1])
```
