...
The distribution is right skewed with an IQR of 617, and a mean of 1500 square feet
...
Answer: 
sample1 is multimodal with no particular skew. It has a sample mean of 1367 square feet, and an IQR of 462.
The mean is below the population mean.
...
mean(samp2)
```
The mean of samp2 is higher than that of samp1 and is therefore closer to the population mean.

Suppose we took two more samples, one of size 100 and one of size 1000. I think the sample of size 1000 will provide a 
...
The distribution of `sample_means50` is approximately unimodal with the mean (1499.066) almost equal to 
the population mean (1500).

The centre of the distribution is at 1500
...
hist(sample_means50_1, breaks = 25)
...
With 50, 000 samples means, the mean (1499.394) approached the population mean in fractions higher than that for 5, 000 samples mean. The distribution became more even in appearance.
...
for(i in 1:5000){
   samp <- sample(area, 50)
   sample_means50[i] <- mean(samp)
    # print(i) : Commented out to preserve space
   }
```
...
Each element represents the mean of 50 elements sampled 100 times from the `area` data set
...
Answer:
When the sample size is larger, the distribution appears to be centrally symmetrical and the means tend toowards the population mean.
...
for(i in 1:50){
  ...
  sample_means_price50[i] <- mean(sample(price,50))
...
hist(sample_means_price50, breaks = 25)

```

my best point estimate of the population mean is the sample mean which is $179, 964
...
The distribution is unimodal with the mean price at $181, 076. I think this value approaches the population mean
...
The mean sale of homes in Ames is likely to be the mean price of `sample_means150` as the distribution became more taller around the population mean
...
The distribution of `sample_mean150` has the smaller spread. We would prefer a distribution with smaller spread if we are concerned with making estimates that are more often close to the true value.
