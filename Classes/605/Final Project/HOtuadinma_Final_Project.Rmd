...
tranData <- read.csv2('train.csv',stringsAsFactors = TRUE,header = T, sep = ',', na.strings = 'NA')
testData <- read.csv2('test.csv',stringsAsFactors = TRUE,header = T, sep = ',', na.strings = 'NA')
...
kable((describe(trn_numeric)), table.attr = "style = \"color: #000;\"") %>% kable_styling(bootstrap_options = c("bordered", 'striped', full_width = T))
...
```{r}
#SalePrice <-((26988.854*testData$OverallQual) + (49.573*testData$GrLivArea) +  (11317.522*testData$GarageCars) + (41.478*testData$GarageArea) -98436.050)
