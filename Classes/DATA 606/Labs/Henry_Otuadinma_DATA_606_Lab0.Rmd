...
author: "Jason Bryer, Henry Otuadinma"
rpubs: "http://rpubs.com/jackv13/463333"
github: "https://github.com/henryvalentine/MSDS2019/raw/master/Classes/DATA%20606/Labs/Henry_Otuadinma_DATA_606_Lab0.Rmd"
output:
  html_document:
    css: ./lab.css
    highlight: pygments
...
```{r}
# to extract just the counts of girls baptised
...
Answer: The number of girls baptised every year grew tremendously from 1660 and never went 
below that number in that year (1660) but there was a very obvious dip in the numbers at 1700 and 
it went up again after 1700
...
plot(arbuthnot$year, proportion_of_boys, type = "l")
```

Observation: The proportio of boys fluctuated over the years but was highest before and immediately after 1660. At 1600, 
the proportion was a bit stable. It was lowest immediately after 1700
...
dim(present)
```
The dimensions of the 'present' dataframe is 63 rows and 6 columns.    
...
The present data has lower number of observations than the Arbuthnot data. In both data sets, the number of boys exceeded the number of girls in all the years => for the past and the present, the number of male children are more than that of the female children for the years under consideration, while the number of babies per year in present data are more than they are in the arbuthnot data.
...
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
   ...
   Answer: 
    ...
    Ratio compares the number of boys to girls each year. This is different from proportion
    
```{r}

present_boys_to_present_girls <- present$boys/present$girls
year <- present$year
plot(x = year, y = present_boys_to_present_girls, type = 'l')

```
    
    What do you see? 
    
    Answer: The proportion of boys to girls was highest between the 1940's and late 1950's. After a brief rise between 
    the 70's and 90's, it has been on the decline, with the lowest occuring in early 90 and early 2000. 
    
    Does Arbuthnot's observation about boys being born in
    greater proportion than girls hold up in the U.S.? Include the plot in your 
    response.
    
    Answer: Arbuthnot's data was gathered from children that were brought for baptism. This might not be a good representation of 
    the total proportion of boys to girls per year as many parents might have not brought there children for baptism 
    but comparing the ratios from his dataset with that of the present data, his observation holds (stands) as number of males 
    at birth exceeds that of females (in the US) resulting in non-zero ratios when the proportions of the two sexes were compared 
    for each year.
...
year_with_max <- present$year[(present$boys + present$girls) == max(present$boys + present$girls)]
year_with_max

```
The year with the most total number of births in the US is 1961 and can be clearly seen from the representation below:

```{r}

  plot(x=present$year, y=present$boys, type="l", ylab = 'Male and Female babies', xlab = 'Year', 
       lwd=2, col="forestgreen", main = 'Number of US Male and Female babies per year')
  lines(x=present$year,y=present$girls, lwd=2, col="orange")
  legend("bottomright", legend=c("boys","girls"), lwd=c(2,2), col=c("forestgreen","orange"))
```
