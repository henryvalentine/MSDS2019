...
#### A case is a single birth entry into the data set. There are 1000 cases in the sample
...
Outliers exist in each of the data sets and thses appear above or beneath the whiskers. The third box (pregnancy duration) has many low outliers, while box 5 (Weight gained by mother) has many outliers above the top whisker.
...
#### The two plots have the same 25th percentile. There are more outliers on the lower base for non-smoking mums than there are for the smoking ones. The plot for non-smokers have higher median and IQR. Maybe there are more cases of non-smokers in the sample than there are for smokers.


```{r}
meanweightFunc <-  function(x){
  mean(x$weight)
...
inference(nc$weight, x = nc$habit,
                est=c("mean"), 
...
##### We are 95% confident that the average length of pregnancies fall between ( 38.1528 , 38.5165 ) weeks.
...
##### We will not reject the null hypothesis because the p-value is higher than aplha (0.05) and the data does not provide clear evidence of any differences existing between the average weight gained by younger mothers and that gained by mature mothers.
...
#### The cutoff age is 35 years. It is evident that the youngest mature mother is 35years and the oldest not mature mother is 34years.
...
#### Considering smoking habit and length of pregnancy, I want to find out if smoking habit affects length of pregnancys. 
I want to test the hypothesis at a=.05

N0: Smoking habit does not affect length of pregnancy
NA: Smoking habit affects length of pregnancy
...
#### The null hypothesis stands because there isn't clear evidence to suggest the length of pregnancies vary between smokers and non-smokers.
