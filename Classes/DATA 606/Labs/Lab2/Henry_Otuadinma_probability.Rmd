...
##### Streak lenght of one means a hit could not be sustained and therefore was followed by a miss

#### What about a streak length of 0?

Answer:
##### Streak lenght of 0 means a miss was followed by another miss
...
Answer:
The distribution is right skewed showing most of his hits occuring at 0 (mode is 0) and median is 1


#### What was his typical streak length? How long was his longest streak of baskets?

Answer:
##### His typical streak length is 0, his longest streak of baskets was 4
...
##### sim_streak has a range from 0 to 5. It is also right skewed and has a typical streak lenght 0 (the mode is 0). It has higher streak sustainability than kobe as it's longest streak of baskets is 5

### What is the typical streak length for this simulated independent shooter with a 45% shooting percentage?

Answer:

##### The typical streak length of sim_basket is 0
    
#### How long is the player's longest streak of baskets in 133 shots?

Answer:
##### The longest streak of baskets in 133 shots is 5
...
To run the simulation again with say 250 shots
```{r}

sim_basket <- sample(outcomes, size = 250, replace = TRUE, prob = c(0.45, 0.55))
sim_streak <- calc_streak(sim_basket)
barplot(table(sim_streak))

```

The distributions are not exactly the same but similar in some ways: the shots are independent of each other, 
they have a mode of 1, both are right skewed but this distribution has a longer streak lenght of 7 as this is affected by the number of shots taken.
...
Answer:
Both distribution shows that the shots are independent of each others as there were more zero streaks, therefore, there is insufficient evidence to establish that Kobe Bryant has a hot hand
